...
## Chạy Và Verify Thực Tế

Repo này được copy vào `Lab_Assignment/`, nhưng môi trường Python dùng để chạy demo nằm ở repo cha:

```bash
cd /Users/takiet/Documents/projects/ai_thuc_chien_2026/repo/Batch02-Day9_Multi-Agent_MCP-A2A/Lab_Assignment
```

### 1. Chạy test Supervisor - Workers

Lệnh:

```bash
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 PYTHONPATH=. /Users/takiet/.pyenv/versions/3.11.15/bin/python -m pytest tests/test_supervisor_workers.py -q
```
...
`PYTEST_DISABLE_PLUGIN_AUTOLOAD=1` được dùng để tránh plugin global của máy can thiệp vào `pytest`.

### 2. Chạy demo end-to-end

Lệnh đã verify chạy thành công trên máy này:

```bash
PYTHONPATH=. ../.venv/bin/python src/task10_generation.py
```

Khi chạy đúng, chương trình sẽ:
- In ra 3 câu hỏi mẫu
- Trả về `A: ...`
- Trả về `[Sources: ...]`

Lưu ý:
- Nếu chưa cài `weaviate`, hệ thống sẽ hiện cảnh báo và fallback sang local retrieval
- Đây không phải lỗi chặn chạy; demo vẫn trả được kết quả

### 3. Cách verify pattern Supervisor - Workers

Kiểm tra nhanh cấu trúc orchestration:

```bash
rg -n "def supervisor|def .*worker|Send\\(" src/supervisor_workers.py
```
...
Task 10 sử dụng LangGraph theo pattern Stage 4 của Day 9:

```text
Question -> Supervisor
              |-- Legal Worker --|
              |-- News Worker ---|-> Evidence Worker -> Aggregator -> Answer
```

- `Supervisor`: định tuyến câu hỏi thành `legal`, `news`, hoặc `mixed`.
- `Legal Worker`: lấy evidence từ văn bản pháp luật.
- `News Worker`: lấy evidence từ nguồn tin tức.
- `Evidence Worker`: gộp, loại trùng và kiểm tra evidence trước generation.
- Legal và News Worker chạy song song bằng LangGraph `Send` với câu hỏi mixed.
- Worker lỗi không làm hỏng toàn bộ request; kết quả còn lại vẫn được tổng hợp.
